...
void Button::Update(const Transformation &t, std::shared_ptr<DisplayObject> instance)
{
    ...
    double xratio = static_cast<double>(m_owner->GetManager()->GetWidth()) / m_owner->GetWidth();
    double yratio = static_cast<double>(m_owner->GetManager()->GetHeight()) / m_owner->GetHeight();
